...
import { getVendor, getVendorByUsername, getVendorTransactionHistory } from '@/lib/firestore';
...
export default function UsernameProfilePage() {
  ...
  async function loadVendor() {
    try {
      ...
      let v = await getVendorByUsername(username);
      
      // If not found, try by ID (fallback for old links that might omit /vendor prefix)
      if (!v) {
        v = await getVendor(username);
      }

      setVendor(v);
      if (v) {
        const h = await getVendorTransactionHistory(v.id, 5);
        setHistory(h);
      }
    } catch (err) {
      console.error(err);
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-[2rem] p-8 sm:p-12 text-center mb-10 shadow-soft border border-slate-100 relative overflow-hidden">
          ...
          <div className="relative inline-block mb-6 mt-4">
            <div className={`w-28 h-28 rounded-[2rem] flex items-center justify-center text-4xl font-black shadow-sm border-4 border-white rotate-3 hover:rotate-0 transition-transform duration-300 overflow-hidden ${vendor.photoURL ? 'bg-white' : 'bg-emerald-100 text-emerald-700'}`}>
              {vendor.photoURL ? (
